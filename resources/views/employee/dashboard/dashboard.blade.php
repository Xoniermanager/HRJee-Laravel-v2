                            ...
                            <div class="card-body py-3">
                                <div class="row">
                                    ...
                                    <div class="col-md-9 d-flex punchinbar" style="gap: 15px">
                                        <p>Now <span id="current-time"></span></p>
                                        <span data-kt-element="bullet"
                                        class="bullet bullet-vertical d-flex align-items-center h-40px bg-primary"
                                        ...
                                        @if($existingAttendanceDetail && !empty($existingAttendanceDetail))
                                            <p> PunchIn Time : <span>{{ date('H:i:s',strtotime($existingAttendanceDetail->punch_in)) }}</span>
                                            </p>
                                        @endif
                                        <span data-kt-element="bullet"
                                        class="bullet bullet-vertical d-flex align-items-center h-40px bg-primary"
                                        ...
                                        @if(!empty($existingAttendanceDetail) && $existingAttendanceDetail->punch_out!='' )
                                            <p> PunchOut Time : <span>{{ date('H:i:s',strtotime($existingAttendanceDetail->punch_out)) }}</span>
                                            </p>
                                            <span data-kt-element="bullet"
                                            class="bullet bullet-vertical d-flex align-items-center h-40px bg-primary"
                                            style="width: 1px;height: 30px !important;"></span>
                                            <p> Working Hours : <span>{{getTotalWorkingHour($existingAttendanceDetail->punch_in, $existingAttendanceDetail->punch_out)}}</span>
                                            </p>
                                        @endif
...
<script>
    ...
    function startTimer(time) {
        ...
        const timerInterval = setInterval(() => {
            ...
            // Display the timer
            $("#break_timer").text(`${String(currentHours).padStart(2, '0')}:${String(currentMinutes).padStart(2, '0')}:${String(currentSeconds).padStart(2, '0')}`);
