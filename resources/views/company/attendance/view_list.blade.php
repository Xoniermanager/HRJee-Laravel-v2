                ...
                @foreach ($employeeDetail['allAttendanceDetails'] as $key => $item)
                    @php
                        ...
                        if (!empty($item['punch_in'])) {
                            ...
                            $title = "Punch In By: " . (!empty($item['punch_in_by']) ? $item['punch_in_by'] : '-') .
                                " | Reason: " . (!empty($item['remark']) ? $item['remark'] : '-');
                        }
                    @endphp
                                    ...
                                    <td>
                                        {{-- @if (!empty($item['id'])) --}}
                                            <a href="javascript:void(0);" class="btn btn-primary btn-sm"
                                                ...
                                                onclick="edit_attendance('{{ $item['id'] ?? ''}}', '{{ !empty($item['punch_in']) ? date('H:i', strtotime($item['punch_in'])) : '' }}', '{{ !empty($item['punch_out']) ? date('H:i', strtotime($item['punch_out'])) : '' }}', '{{ \Carbon\Carbon::parse($key)->format('Y-m-d') }}')">
