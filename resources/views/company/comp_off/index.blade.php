    ...
    <div class="content d-flex flex-column flex-column-fluid fade-in-image" id="kt_content">
                    ...
                    <div class="card-header cursor-pointer p-0">
                        ...
                        <div class="card-title m-0">
                            ...
                            <select name="status" class="form-control min-w-150px me-2" id="status">
                                ...
                                <option {{ old('status') == 'accepted' || request()->get('status') == 'accepted' ? 'selected' : '' }}
        ...
        <script>
            function handleStatus(ele, requestId) {
                $.ajax({
                    url: "{{ route('comp-off.statusUpdate') }}",
                    type: 'get',
                    data: {
                        'requestId': requestId,
                        'status': ele.value,
                    ...
                    success: function (res) {
                        if (res.status) {
                            ...
                            $('#comp_off_list').replaceWith(res.data);
                        } else {
            ...
            function search_filter_results() {
                $.ajax({
                    ...
                    success: function (response) {
                        $('#comp_off_list').replaceWith(response.data);
                    }
                });
            }
        </script>
