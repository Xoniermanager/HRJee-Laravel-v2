...
@section('content')
@section('title', 'Address Request Management')
                ...
                <div class="row gy-5 g-xl-10">
                    ...
                    <div class="card custom-table p-0">
                        ...
                        <div class="mb-5 mb-xl-10">
                            @include('company.address_request.list')
                        </div>
                        <!--end::Col-->
                    </div>
                    <!--end::Row-->
            ...
            <script>
                ...
                Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{ route('address.request.statusUpdate') }}",
                                type: 'get',
                                data: {
                                    'requestId': requestId,
                                    'status': ele.value,
                ...
                function search_filter_results() {
                    $.ajax({
                        type: 'GET',
                        url: company_ajax_base_url + '/address-request/search/filter',
                        data: {
                            'status': $('#status').val(),
                            'search': $('#search').val()
                        },
