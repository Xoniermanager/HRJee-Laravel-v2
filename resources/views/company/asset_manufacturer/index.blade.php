...
<div class="content d-flex flex-column flex-column-fluid fade-in-image" id="kt_content">
    ...
    <script>
        ...
        jQuery(document).ready(function($) {
            jQuery("#asset_manufacturer_form").validate({
                rules: {
                    name: "required",
                },
                messages: {
                ...
                submitHandler: function(form) {
                    var asset_manufacturer_data = $(form).serialize();
                    $.ajax({
                        url: "{{ route('asset.manufacturer.store') }}",
                        type: 'POST',
                        data: asset_manufacturer_data,
                        success: function(response) {
                            jQuery('#add_asset_manufacturer').modal('hide');
                            swal.fire("Done!", response.message, "success");
                            $('#asset_manufacturer_list').replaceWith(response.data);
                            jQuery("#asset_manufacturer_form")[0].reset();
            ...
            jQuery("#asset_manufacturer_update_form").validate({
                rules: {
                    name: "required"
                },
                messages: {
                ...
                submitHandler: function(form) {
                    var asset_manufacturer_data = $(form).serialize();
                    $.ajax({
                        url: "<?= route('asset.manufacturer.update') ?>",
                        type: 'post',
                        data: asset_manufacturer_data,
                        success: function(response) {
                            jQuery('#edit_asset_manufacturer').modal('hide');
                            swal.fire("Done!", response.message, "success");
                            jQuery('#asset_manufacturer_list').replaceWith(response.data);
                        },
                        error: function(error_messages) {
                            let errors = error_messages.responseJSON.error;
                            for (var error_key in errors) {
        ...
        function search_filter_results() {
            $.ajax({
                type: 'GET',
                url: company_ajax_base_url + '/asset-manufacturer/search/filter',
                data: {
                    'status': $('#status').val(),
                    'search': $('#search').val()
                },
