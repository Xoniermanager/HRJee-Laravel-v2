                        ...
                        <div class="row">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 form-group">
                                        ...
                                        <select class="form-control alldetails" id="l_country_id" name="l_country_id">
                                            ...
                                            <option {{ old('l_country_id')==$countriesDetails->id ? 'selected' : '' }}
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="row">
                                    <div class="col-md-12 form-group">
                                        <label for="">Address *</label>
                                        <textarea class="form-control" type="text" name="p_address"
                                            id="p_address"></textarea>
                                    </div>
                                    <div class="col-md-6 form-group">
                                        <label for="">Country *</label>
                                        <select class="form-control" id="p_country_id" name="p_country_id" {{
                                            ...
                                            <option value="">Please Select Country</option>
                                            @forelse ($allCountries as $countriesDetails)
                                            <option {{ old('p_country_id')==$countriesDetails->id ? 'selected' : '' }}
                                                value="{{ $countriesDetails->id }}">
                                                {{ $countriesDetails->name }}</option>
                                            @empty
                                            <option value="">No Country Found</option>
...
<script>
    function getAddressDetails(userId) {
         ...
         $.ajax({
             ...
             {
                 $('#address_user_id').val(response.data[0].user_id);
                 if (response.data[0].address_type == 'both_same')
                 {
                    let value = response.data[0];
                     ...
                     var p_state_id = 'p_state_id';
                     get_all_state_using_country_id(value.country_id, l_div_id,value.state_id);
                     get_all_state_using_country_id(value.country_id, p_state_id,value.state_id);
                     get_all_present_address_details();
                 }
                 else {
                    let value = response.data[0];
                    let value1 = response.data[1];
                    $('#address_type').val('0');
                    $('#l_address').val(value.address);
                    $('#l_country_id').val(value.country_id);
                    $('#l_city').val(value.city);
                    $('#l_pincode').val(value.pin_code);
                    $('#p_address').val(value1.address);
                    $('#p_country_id').val(value1.country_id);
                    $('#p_city').val(value1.city);
                    $('#p_pincode').val(value1.pin_code);
                    var l_div_id = 'l_state_id';
                    if (l_state_id && l_country_id) {
                    get_all_state_using_country_id(value.country_id, l_div_id, value.state_id);}
                    var p_div_id = 'p_state_id';
                    if (p_state_id && p_country_id) {
                    get_all_state_using_country_id(value1.country_id, p_div_id, value1.state_id);
                    }
                 }
                }
            });
