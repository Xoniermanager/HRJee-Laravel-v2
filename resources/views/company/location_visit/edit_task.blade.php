		...
		<div class="col-lg-12 col-xl-12 col-xxl-12 mb-5">
			...
			<div class="card h-md-100">
				...
				<div class="card-header align-items-center p-0">
					<div class="card-body">
						<form action="{{ route('location_visit.update_task_assign', $taskdetails->id) }}" method="POST"
							...
							<div class="row">
								<div class="col-md-3 form-group">
									...
									<select name="user_id" id="" class="form-control">
										...
										@foreach ($allEmployeeDetails as $item)
											<option value="{{ $item->id }}" {{ $taskdetails->user_id == $item->id ? 'selected' : '' }}>
												{{ $item->name }}</option>
										@endforeach
									</select>
								...
								<div class="col-md-3 form-group">
									...
									<select name="user_end_status" id="" class="form-control">
										...
										<option value="processing" {{ $taskdetails->user_end_status == 'processing' ? 'selected' : '' }}>Processing
										</option>
										<option value="rejected" {{ $taskdetails->user_end_status == 'rejected' ? 'selected' : '' }}>Rejected</option>
										<option value="completed" {{ $taskdetails->user_end_status == 'completed' ? 'selected' : '' }}>Completed
										</option>
								...
								<div class="col-md-3 form-group">
									...
									<select name="final_status" id="" class="form-control">
										...
										<option value="processing" {{ $taskdetails->final_status == 'processing' ? 'selected' : '' }}>Processing
										</option>
										<option value="rejected" {{ $taskdetails->final_status == 'rejected' ? 'selected' : '' }}>Rejected</option>
										<option value="completed" {{ $taskdetails->final_status == 'completed' ? 'selected' : '' }}>Completed
										</option>
									</select>
								</div>
									...
									@foreach ($fieldDetails->formfield as $item)
										...
										@if ($item->type == 'textarea')
											<div class="col-md-3 form-group">
												...
												<textarea name="{{ strtolower($item->label) }}" id="" class="form-control" required>{{ $formDetails[strtolower($item->label)] ?? '' }}</textarea>
											</div>
											...
											<div class="col-md-3 form-group">
												...
												<select name="{{ strtolower($item->label) }}" id="" class="form-control" required>
													...
													@foreach ($options as $key => $value)
														<option value="{{ $key }}"
															{{ $formDetails[strtolower($item->label)] == $key ? 'selected' : '' }}>
															{{ $value }}
														</option>
													@endforeach
												</select>
										...
										@elseif($item->type == 'radio')
											<div class="col-md-3 form-group">
												...
												<div>
													@foreach ($options as $key => $value)
														<input type="radio" name="{{ strtolower($item->label) }}" class="mt-4" value="{{ $key }}"
															{{ $formDetails[strtolower($item->label)] == $key ? 'checked' : '' }}><strong>&nbsp;{{ $value }}</strong>
													@endforeach
												</div>
											</div>
										@else
											<div class="col-md-3 form-group">
												...
												<input type="{{ $item->type }}" name="{{ strtolower($item->label) }}" id=""
													class="form-control" required value="{{ $formDetails[strtolower($item->label)] ?? '' }}">
											</div>
										@endif
									@endforeach
								@endif
								<div class="col-md-6">
									...
									<input value="{{ $taskdetails->visit_address }}" type="text" name="visit_address" id="address"
										class="form-control" placeholder="Enter an address" oninput="initAutocomplete('address')" />
