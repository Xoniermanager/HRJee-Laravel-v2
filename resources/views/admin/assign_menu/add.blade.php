...
@section('content')
											...
											<div class="col-md-12">
												<ul>
													@foreach ($allMenus as $menu)
														<li>
															<input type="checkbox" name="menu_id[]" value="{{ $menu->id }}" id="parentCheck_{{ $menu->id }}"
	...
	<script>
		...
		function getPermission(value) {
			...
			$.ajax({
				url: "<?= route('admin.company.getPermission') ?>",
				type: 'GET',
				data: {
				...
				success: function(response) {
					...
					// ✅ After setting, update the "All" checkbox
		...
		function updateAllCheckboxState() {
			const total = $('input[name="menu_id[]"]').length;
			const checked = $('input[name="menu_id[]"]:checked').length;
		...
		$(document).ready(function() {
			...
			// Auto select company if editing permissions for a company
			const urlParams = new URLSearchParams(window.location.search);
			const companyId = urlParams.get('id');
			if (companyId) {
				$('select[name="company_id"]').val(companyId).change();
			}
