...
@section('content')
    ...
    $(document).on("input", "#search", function(e) {
        ...
        {
            $.ajax({
                    url: "{{ route('admin.document.type.search') }}",
                    type: 'get',
                    data: {
                        'key': key,
                        'status': status,
        ...
        jQuery(document).ready(function($) {
            jQuery("#document_type_form").validate({
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        url: "{{ route('admin.document.type.store') }}",
                        type: 'POST',
        ...
        function deleteFunction(id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "<?= route('admin.document.type.delete') ?>",
                        type: "get",
                        data: {
                            id: id
                        },
        ...
        jQuery("#document_type_update_form").validate({
            ...
            submitHandler: function(form) {
                var document_type_data = $(form).serialize();
                $.ajax({
                    url: "<?= route('admin.document.type.update') ?>",
                    type: 'post',
                    data: document_type_data,
                    success: function(response) {
        ...
        function handleStatus(id) {
            ...
            console.log(status);
            $.ajax({
                url: "{{ route('admin.document.type.statusUpdate') }}",
                type: 'get',
                data: {
                    'id': id,
                    'status': status,
