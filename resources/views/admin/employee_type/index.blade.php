...
@section('content')
    ...
    $(document).on("input", "#search", function(e) {
        ...
        {
            $.ajax({
                    url: "{{ route('admin.employee_type.search') }}",
                    type: 'get',
                    data: {
                        'key': key,
                        'status': status,
        ...
        jQuery(document).ready(function($) {
            jQuery("#employee_type_form").validate({
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        url: "{{ route('admin.employee_type.store') }}",
                        type: 'POST',
        ...
        function deleteFunction(id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "<?= route('admin.employee_type.delete') ?>",
                        type: "get",
                        data: {
                            id: id
                        },
        ...
        jQuery("#employee_type_update_form").validate({
            ...
            submitHandler: function(form) {
                var employee_type_data = $(form).serialize();
                $.ajax({
                    url: "<?= route('admin.employee_type.update') ?>",
                    type: 'post',
                    data: employee_type_data,
                    success: function(response) {
        ...
        function handleStatus(id) {
            ...
            console.log(status);
            $.ajax({
                url: "{{ route('admin.employee_type.statusUpdate') }}",
                type: 'get',
                data: {
                    'id': id,
                    'status': status,
