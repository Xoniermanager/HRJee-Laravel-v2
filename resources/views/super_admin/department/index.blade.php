...
@section('content')
                            ...
                            <div class="d-md-flex d-sm-block">
                                ...
                                <div class="flex-grow-1 text-end">
                                    <button class="btn btn-primary mx-auto mt-3" type="button" data-bs-toggle="modal"
                                        data-bs-target="#add_department">Add</button>
                                </div>
                            </div>
                        </div>
                    @include('super_admin.department.department_list')
                ...
                <div class="modal-toggle-wrapper">
                    ...
                    <form class="row g-3" id="department_update_form">
                        ...
                        <div class="col-md-12">
                            ...
                            <input class="form-control" type="text" placeholder="Enter Your Department Name" name="name" id="name">
                                @error('name')
                ...
                <div class="modal-body social-profile text-start">
                    <div class="modal-toggle-wrapper">
                        ...
                        <form class="row g-3" id="department_form">
                            ...
                            <div class="col-md-12">
                                <label class="form-label" >Name</label>
                                <input class="form-control"  type="text" name="name" placeholder="Enter Your Department Name" id="name">
                                    @error('name')
                                        <span  class="text-denger">{{ $message}} </span>
                                    @enderror
                            </div>
    ...
    $(document).on("input", "#search", function(e) {
        var key = $(this).val();
        search_departments(key,status= null)
        });

        $('#filterByStatus').change(function(){
            var status= $(this).val();
            search_departments(key=null,status)
        });

        function search_departments(key,status)
        {
            $.ajax({
                    url: "{{ route('super_admin.department.search') }}",
                    type: 'get',
                    data: {
                        'key': key,
                        'status': status,
                    ...
                    success: function(res) {
                        if (res) {
                            jQuery('#department_list').replaceWith(res.data);
                        } 
                    }
                })
        }
        ...
        jQuery(document).ready(function($) {
            jQuery("#department_form").validate({
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        url: "{{ route('super_admin.department.store') }}",
                        type: 'POST',
        ...
        function deleteFunction(id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "<?= route('super_admin.department.delete') ?>",
                        type: "get",
                        data: {
                            id: id
                        },
        ...
        jQuery("#department_update_form").validate({
            rules: {
                name: "required"
            },
            messages: {
            ...
            submitHandler: function(form) {
                var department_data = $(form).serialize();
                $.ajax({
                    url: "<?= route('super_admin.department.update') ?>",
                    type: 'post',
                    data: department_data,
                    success: function(response) {
        ...
        function handleStatus(id) {
            ...
            console.log(status);
            $.ajax({
                url: "{{ route('super_admin.department.statusUpdate') }}",
                type: 'get',
                data: {
                    'id': id,
                    'status': status,
