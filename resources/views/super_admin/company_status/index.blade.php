...
@section('content')
    ...
    $(document).on("input", "#search", function(e) {
        ...
        {
            $.ajax({
                    url: "{{ route('super_admin.company.status.search') }}",
                    type: 'get',
                    data: {
                        'key': key,
                        'status': status,
        ...
        jQuery(document).ready(function($) {
            jQuery("#company_status_form").validate({
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        url: "{{ route('super_admin.company.status.store') }}",
                        type: 'POST',
        ...
        function deleteFunction(id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "<?= route('super_admin.company.status.delete') ?>",
                        type: "get",
                        data: {
                            id: id
                        },
        ...
        jQuery("#company_status_update_form").validate({
            ...
            submitHandler: function(form) {
                var company_status_data = $(form).serialize();
                $.ajax({
                    url: "<?= route('super_admin.company.status.update') ?>",
                    type: 'post',
                    data: company_status_data,
                    success: function(response) {
        ...
        function handleStatus(id) {
            ...
            $.ajax({
                url: "{{ route('super_admin.company.status.statusUpdate') }}",
                type: 'get',
                data: {
                    'id': id,
                    'status': status,
