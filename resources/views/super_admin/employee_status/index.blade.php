...
@section('content')
    ...
    $(document).on("input", "#search", function(e) {
        ...
        {
            $.ajax({
                    url: "{{ route('super_admin.document.employee_status.search') }}",
                    type: 'get',
                    data: {
                        'key': key,
                        'status': status,
        ...
        jQuery(document).ready(function($) {
            jQuery("#employee_status_form").validate({
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        url: "{{ route('super_admin.employee_status.store') }}",
                        type: 'POST',
        ...
        function deleteFunction(id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "<?= route('super_admin.employee_status.delete') ?>",
                        type: "get",
                        data: {
                            id: id
                        },
        ...
        jQuery("#employee_status_update_form").validate({
            ...
            submitHandler: function(form) {
                var employee_status_data = $(form).serialize();
                $.ajax({
                    url: "<?= route('super_admin.employee_status.update') ?>",
                    type: 'post',
                    data: employee_status_data,
                    success: function(response) {
        ...
        function handleStatus(id) {
            ...
            console.log(status);
            $.ajax({
                url: "{{ route('super_admin.employee_status.statusUpdate') }}",
                type: 'get',
                data: {
                    'id': id,
                    'status': status,
