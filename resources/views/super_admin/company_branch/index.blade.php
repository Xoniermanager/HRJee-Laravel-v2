...
{{$allCompanyBranchDetails}}
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-12">
                    ...
                    @include('super_admin.company_branch.company_branch_list')
                    ...
                    <div class="modal-toggle-wrapper">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                ...
                                @forelse ($states as $state)
                                    <option
                                        {{ $local->state_id ?? old('l_state_id') == $state->id ? 'selected' : '' }}
                                        value="{{ $state->id }}">
                                        {{ $state->name }}</option>
                                @empty
                                    <option value="">No state Found</option>
    ...
    $(document).on("input", "#search", function(e) {
        ...
        {
            $.ajax({
                    url: "{{ route('super_admin.company.branch.search') }}",
                    type: 'get',
                    data: {
                        'key': key,
                        'status': status,
        ...
        jQuery(document).ready(function($) {
            jQuery("#company_branch_form").validate({
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        url: "{{ route('super_admin.company.branch.store') }}",
                        type: 'POST',
        ...
        function deleteFunction(id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "<?= route('super_admin.company.branch.delete') ?>",
                        type: "get",
                        data: {
                            id: id
                        },
        ...
        jQuery("#company_branch_update_form").validate({
            ...
            submitHandler: function(form) {
                var company_branch_data = $(form).serialize();
                $.ajax({
                    url: "<?= route('super_admin.company.branch.update') ?>",
                    type: 'post',
                    data: company_branch_data,
                    success: function(response) {
        ...
        function handleStatus(id) {
            ...
            console.log(status);
            $.ajax({
                url: "{{ route('super_admin.company.branch.statusUpdate') }}",
                type: 'get',
                data: {
                    'id': id,
                    'status': status,
