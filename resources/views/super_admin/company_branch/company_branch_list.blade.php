<div  id="company_branch_list" class="card-body">
                ...
                <table class="table nowrap" id="project-status">
                    <thead>
                        <tr>
                            ...
                            <th>
                                <span class="f-light f-w-600">Company Branch</span>
                            </th>
                            <th>
                        ...
                        @forelse ($allCompanyBranchDetails as $key => $companyBranchDetails)
                            <tr>
                                <td>
                                ...
                                <td>
                                        <p class="f-light">{{ $companyBranchDetails->name }}</p>
                                </td>
                                <td>
                                    <p class="f-light">{{ $companyBranchDetails->email }}</p>
                            </td>
                            <td>
                                <p class="f-light">{{ $companyBranchDetails->contact_no }}</p>
                        </td>

                                <td>
                                    <p class="f-light">{{ $companyBranchDetails->address }}</p>
                            </td>
                                <td>
                                    <div class="form-check form-switch form-check-inline">
                                            <input type="checkbox" <?= $companyBranchDetails->status == '1' ? 'checked' : '' ?>
                                            onchange="handleStatus({{ $companyBranchDetails->id }})" id="checked_value_{{ $companyBranchDetails->id }}" class="form-check-input switch-info check-size">
                                ...
                                <td>
                                    <div class="product-action"><a href="#" data-bs-toggle="modal"
                                        data-bs-target="#edit_company_size"
                                        onClick="edit_company_branch_details('{{$companyBranchDetails}}')">
                                         ...
                                         <a href="#" onclick="deleteFunction('{{ $companyBranchDetails->id }}')">   
                ...
                @empty
                    <td colspan="3">
                        <span class="text-danger">
                            <strong>No company branch Found!</strong>
                        </span>
                    </td>
                    @endforelse
                </table>
