...
@section('content')
    ...
    <script>
        ...
        $(document).ready(function() {
            $('.companyAccountBtn').click(function(e) {
                ...
                if ($("#companyAccountform").valid()) {
                    //  var company_detail = $('#companyAccountform').serialize();
                    var form = $('#companyAccountform')[0];
                    var formData = new FormData(form);
                    var formData = {};
                    for (var i = 0; i < form.elements.length; i++) {
                        var fieldName = form.elements[i].name;
                        var fieldValue = form.elements[i].value;
                        if (fieldName && fieldValue && form.elements[i].type !== 'button') {
                            formData[fieldName] = fieldValue;
                        }
                    }

                    $.ajax({
                        url: "<?= route('super_admin.company.store') ?>",
                        type: 'post',
                        data: formData,
            ...
            if ($("#companyDetailForm").valid()) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        window.location.href = "http://localhost:9000/super-admin/company";
                    },
                    error: function(error_messages) {
        ...
        $('.previousPersonDetailBtn').click(function(e) {
            ...
            $('#add-product-pills-tab a[href="#gallery-product"]').tab('show');
        });
    </script>
    <style>
        .bottomspace {
