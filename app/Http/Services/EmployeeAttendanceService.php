...
class EmployeeAttendanceService
{
  ...
  public function __construct(EmployeeAttendanceRepository $employeeAttendanceRepository, UserDetailServices $userDetailRepository,LeaveService $leaveService,HolidayServices $holidayService)
  {
  ...
  {
    $userId = Auth()->guard('employee')->user()->id ?? auth()->guard('employee_api')->user()->id;
    $companyId = Auth()->guard('employee')->user()->company_id ?? auth()->guard('employee_api')->user()->company_id;
    ...
    if (isset($existingDetails) && !empty($existingDetails)) {
      ...
      $todayConfirmLeaveDeatils = $this->leaveService->getUserConfirmLeaveByDate($userId,date('Y-m-d'));
      ...
      $todayHoliday = $this->holidayService->getHolidayByDate($companyId,date('Y-m-d'));
      dd($todayHoliday);
      if($todayHoliday){
