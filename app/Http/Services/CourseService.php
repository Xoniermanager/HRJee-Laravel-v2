...
class CourseService
{
    ...
    public function fetchCoursesByCompanyID($companyIDs) {
        $allCourses = $this->courseRepository->whereIn('created_by', $companyIDs);
    ...
    {
        $allCompanyDetails = $this->courseRepository->where('type', 'company');
    ...
    {
        $allEmployeeDetails = $this->courseRepository
            ->where('type', 'user')
    ...
    {
        $allManagers = $this->courseRepository->where('role_id', '!=', null)->where('type', 'user')->with(['details' => function ($query) use ($branchIDs) {
    ...
    public function getCompanyEmployeeIDs($companyId) {
        return $this->courseRepository->where('company_id', $companyId)->pluck('id')->toArray();
    }
}
